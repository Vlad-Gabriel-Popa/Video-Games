...
void Laborator5::Update(float deltaTimeSeconds)
{
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(5, 2.5f, 0));
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(4, 6.0f, 1));
...
void Laborator5::OnInputUpdate(float deltaTime, int mods)
{
	...
	if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
	{
		...
		if (window->KeyHold(GLFW_KEY_J)) {
			width += deltaTime * 10.f;
			projectionMatrix = glm::ortho(-1.5f - width/2, 1.5f + width/2, -1.5f, 1.5f, 2.f, 1000.f);
		}
...
void Laborator5::OnKeyPress(int key, int mods)
{
	...
	if (key == GLFW_KEY_O)
	{
		projectionMatrix = glm::ortho(-1.5f, 1.5f, -1.5f, 1.5f, 2.f, 1000.f);
	}
	if (key == GLFW_KEY_P)
	{
		projectionMatrix = glm::perspective(RADIANS(60), window->props.aspectRatio, 0.01f, 200.0f);
